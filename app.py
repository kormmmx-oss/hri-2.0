...
# --- [1. 설정 및 7개 사례 데이터 정의] ---
st.set_page_config(page_title="전북 극한호우 감시 & 7대 사례 시뮬레이터", layout="wide")
...
# 제공해주신 25년 통계 기반 7개 핵심 사례 (HRI 2.1용)
PAST_RECORDS = {
...
# 지역별 지형 보정 계수
LOCATION_MODIFIERS = {
    "군산": 1.05, "부안": 1.03, "고창": 1.02, "익산": 1.0, "김제": 1.0, "전주": 1.0, 
    "완주": 0.98, "정읍": 0.97, "임실": 0.95, "남원": 0.96, "순창": 0.94, 
    "진안": 0.92, "무주": 0.91, "장수": 0.90
}

# --- [2. 핵심 엔진: HRI 2.1 지형 보정] ---
...
with t1:
    ...
    st.subheader("전북 극한호우 7대 사례 시뮬레이터 (HRI 2.1)")
with t2:
...
if mode == "실시간 감시":
    ...
    sim_case = st.selectbox("📅 시뮬레이션 사례 선택 (총 7개)", list(PAST_RECORDS.keys()))
    cd = PAST_RECORDS[sim_case]
    weather_source = {}
    for name in LOCATIONS.keys():
        # 지역별 미세 변동 및 지형 보정 시뮬레이션 데이터 생성
...
target = b1.selectbox("🎯 상세 분석 지역 선택", list(LOCATIONS.keys()))
...
with b3:
    if tw:
        if mode == "실시간 감시":
            ...
            st.metric("🧬 사례 CAPE", f"{tw['T1H']*100:.0f} J/kg")
            st.metric("🌊 사례 PWAT", f"{tw['REH']*0.65+10:.1f} mm")
