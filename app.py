...
# --- [1. 설정 및 7대 사례 데이터 정의] ---
st.set_page_config(page_title="전북 극한호우 실시간 감시 & 7대 사례 시뮬레이터", layout="wide")
...
# 전북 25년 통계 기반 7대 핵심 사례 (전수 등록)
PAST_RECORDS = {
    "2025-09-07 군산 (152.2mm)": {"pwat": 65.5, "cape": 4500, "v850": 20.0, "up": 40.2, "ki": 38.0, "rn1": 152.2, "img": "https://img.kma.go.kr"},
    "2024-07-10 익산 (125.5mm)": {"pwat": 64.5, "cape": 3800, "v850": 23.5, "up": 18.3, "ki": 37.0, "rn1": 125.5, "img": "https://img.kma.go.kr"},
    "2024-08-26 남원 (110.0mm)": {"pwat": 62.0, "cape": 3200, "v850": 18.0, "up": 25.0, "ki": 36.5, "rn1": 110.0, "img": "https://img.kma.go.kr"},
    "2022-08-11 군산 (100.0mm)": {"pwat": 59.7, "cape": 1500, "v850": 21.0, "up": 15.2, "ki": 35.8, "rn1": 100.0, "img": "https://img.kma.go.kr"},
    "2020-07-30 완주 (100.5mm)": {"pwat": 60.5, "cape": 1850, "v850": 12.7, "up": 7.9, "ki": 36.0, "rn1": 100.5, "img": "https://img.kma.go.kr"},
    "2017-07-15 군산 (93.5mm)": {"pwat": 58.2, "cape": 2100, "v850": 15.5, "up": 12.0, "ki": 35.5, "rn1": 93.5, "img": "https://img.kma.go.kr"},
    "2012-08-13 군산 (110.0mm)": {"pwat": 68.2, "cape": 4200, "v850": 19.8, "up": 30.1, "ki": 40.0, "rn1": 110.0, "img": "https://img.kma.go.kr"}
}
...
def get_hri_21(pwat, cape, v850, loc_name, rain_1h=0, is_sim=False, up=15.0, ki=32.0):
    stability = 0.3 if (not is_sim and rain_1h <= 0) else 1.0
    mod = LOCATIONS[loc_name]["mod"]
    score = ((pwat*ki)/2300*30 + (cape/4000)*35 + (v850*up)/750*35) * 1.02 * mod * stability
...
with tab2:
    sim_case = st.selectbox("분석할 과거 사례를 선택하세요 (총 7개)", list(PAST_RECORDS.keys()))
    cd = PAST_RECORDS[sim_case]
    # 지역별 미세 변동이 포함된 시나리오 데이터 생성
    sim_weather = {}
    for name in LOCATIONS.keys():
        v = 1 + (hash(name) % 8 - 4) / 100 
        sim_weather[name] = {'T1H': (cd['cape']*v)/100, 'REH': ((cd['pwat']*v)-10)/0.65, 'WSD': (cd['v850']*v)/2.5, 'RN1': cd['rn1']*v, 'sim': True, 'up': cd['up'], 'ki': cd['ki']}
...
with m1:
    ...
    for name, info in LOCATIONS.items():
        w = source.get(name)
        if w:
            sc = get_hri_21(w['REH']*0.65+10, w['T1H']*100, w['WSD']*2.5, name, rain_1h=w['RN1'], is_sim=is_sim_mode, up=w.get('up', 15.0), ki=w.get('ki', 32.0))
...
with m2:
    ...
    for n in LOCATIONS.keys():
        w = source.get(n)
        sc = get_hri_21(w['REH']*0.65+10, w['T1H']*100, w['WSD']*2.5, n, rain_1h=w['RN1'] if w else 0, is_sim=is_sim_mode, up=w.get('up', 15.0) if w else 15.0, ki=w.get('ki', 32.0) if w else 32.0) if w else 0
        summary.append({"지역": n, "지수": sc, "1h 강수": round(w['RN1'], 1) if w else 0})
...
with b1:
    ...
    tsc = get_hri_21(tw['REH']*0.65+10, tw['T1H']*100, tw['WSD']*2.5, target, rain_1h=tw['RN1'] if tw else 0, is_sim=is_sim_mode, up=tw.get('up', 15.0) if tw else 15.0, ki=tw.get('ki', 32.0) if tw else 32.0) if tw else 0
